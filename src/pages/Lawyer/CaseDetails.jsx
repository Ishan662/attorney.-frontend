// >> In your existing file: pages/CaseDetails.jsx
...
import { getCaseById } from '../../services/caseService'; // To fetch the main case data
    ...
    useEffect(() => {
        ...
        const fetchCaseDetails = async () => {
            try {
                ...
                setCaseData(data); // Store the main case data
                setHearings(data.hearings || []); // Store the hearings list from the fetched data
            } catch (err) {
        ...
        fetchCaseDetails();
    }, [caseId]); // Re-run if the caseId changes

    // --- MODAL AND HEARING LOGIC (LOCAL STATE ONLY) ---
    const handleAddHearing = (newHearing) => {
        // This function now only updates the LOCAL state for an immediate UI update.
        // NOTE: This new hearing is NOT saved to the backend yet. You would add a
        // service call here later (e.g., `createHearing(caseId, newHearing)`)
        const updatedHearings = [...hearings, { ...newHearing, status: 'Planned' }];
        setHearings(updatedHearings);
        alert("New hearing date added to view. Backend saving not yet implemented.");
    };
